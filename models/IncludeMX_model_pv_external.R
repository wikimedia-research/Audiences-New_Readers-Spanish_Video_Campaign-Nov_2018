...
# eswiki external pv from Mexico

cat("Data preparation...\n")
test_series <- pageviews %>%
  filter(country_code == 'MX', project == 'es.wikipedia', referer_class == "external") %>%
  group_by(date) %>%
  summarize(MX_es.wikipedia = sum(pageviews)) %>%
  ungroup() %>%
  complete(date, fill = list(MX_es.wikipedia = 0))
control_series <- pageviews %>%
  filter(!(country_code == 'MX' & project == 'es.wikipedia'), referer_class == "external") %>%
  group_by(date, country_code, project) %>%
  summarize(pageviews = sum(pageviews)) %>%
  ungroup() %>%
  complete(date, nesting(country_code, project), fill = list(pageviews = 0)) %>%
  unite(metrics, country_code, project) %>%
...
preselect_controls <- grep("^MX_", colnames(control_series), value = TRUE)
...
model_compare <- foreach(i=1:nrow(model_spec), .combine=rbind,
  ...
  cv_results <- bsts_cv_loop(x, y, train_length = param$train_length, cv_end = online_start - 1, horizon = 70, nfold = 10, step = 42,
                             ...
                             niter = 5000, n_control_candidates = n_control_candidates, preselect_controls = preselect_controls)
...
save(model_compare, file = "data/include_mx/eswiki_external_pv_model_compare.RData")

# Check model compare results
load("data/include_mx/eswiki_external_pv_model_compare.RData")
...
# best model is 13, MAPE = 0.07151976
...
selected_controls <- unique(c(head(selected_controls$dtw_all, n_control_candidates), head(selected_controls$dtw_eswiki, n_control_candidates),
  head(selected_controls$corr_all, n_control_candidates), head(selected_controls$corr_eswiki, n_control_candidates), preselect_controls))
...
p <- plot(model_impact) +
  ...
  labs(title = "Impact of the video campaign in 10 weeks on Spanish Wikipedia externally-referred page views from Mexico",
  subtitle = "Vertical dashed line represents the start date of the campaign 15 November 2018")
ggsave("eswiki_external_pv_impact.png", plot = p, path = 'docs/figures/include_mx', units = "in", dpi = 300, height = 6, width = 14)
summary(model_impact)
summary(model_impact, "report")
png('docs/figures/include_mx/eswiki_external_pv_inclusion_prob.png', units = "in", res = 300, height = 8, width = 10)
...
sort(colMeans(model$model$coefficients[-(1:burn),] != 0))
    # DO_es.wikipedia     MX_es.wiktionary      MX_en.wikipedia MX_commons.wikimedia
    #       0.98509851           0.99739974           0.99869987           0.99949995
    #  MX_es.wikibooks
    #       1.00000000
...
sort(apply(model$model$coefficients[-(1:burn),], 2, PositiveMean))
    # DO_es.wikipedia     MX_es.wiktionary      MX_en.wikipedia MX_commons.wikimedia
    #    0.239966906           0.344694997           0.211212192           0.196860939
    #  MX_es.wikibooks
    #       0.423028872
