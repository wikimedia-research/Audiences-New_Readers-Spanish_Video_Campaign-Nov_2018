...
# eswiki first visit ud from Mexico

cat("Data preparation...\n")
test_series <- unique_devices %>%
  filter(country_code == 'MX', project == 'es.wikipedia', type == 'first_visit') %>%
  group_by(date) %>%
...
preselect_controls <- grep("^Mexico_", colnames(control_series), value = TRUE)
...
save(model_compare, file = "data/include_mx/eswiki_first_visit_ud_model_compare.RData")

# Check model compare results
load("data/include_mx/eswiki_first_visit_ud_model_compare.RData")
...
# best model is 17 MAPE=0.04483590
...
p <- plot(model_impact) +
  ...
  labs(title = "Impact of the video campaign in 10 weeks on Spanish Wikipedia first-time visit unique devices* from Mexico",
  subtitle = "Vertical dashed line represents the start date of the campaign 15 November 2018",
  caption = "*Devices that didn't visit at least in the past 30 days, or those that cleared their cookies.")
ggsave("eswiki_first_visit_ud_impact.png", plot = p, path = 'docs/figures/include_mx', units = "in", dpi = 300, height = 6, width = 14)
summary(model_impact)
summary(model_impact, "report")
png('docs/figures/include_mx/eswiki_first_visit_ud_inclusion_prob.png', units = "in", res = 300, height = 8, width = 10)
...
sort(colMeans(model$model$coefficients[-(1:burn),] != 0))
        # United.States_es.wikipedia
        #            0.98329833
        #      Mexico_en.wikipedia  Dominican.Republic_es.wikipedia             Mexico_es.wiktionary
        #               0.99789979                       1.00000000                       1.00000000
        #      Mexico_es.wikibooks Dominican.Republic_es.wiktionary
...
sort(apply(model$model$coefficients[-(1:burn),], 2, PositiveMean))
       # United.States_es.wikipedia
        #            0.0758898228
        #      Mexico_en.wikipedia  Dominican.Republic_es.wikipedia             Mexico_es.wiktionary
        #               0.2883038568                       0.3200871466                       0.4155766292
        #      Mexico_es.wikibooks Dominican.Republic_es.wiktionary
        #               0.3739699408                       -0.3618480860
