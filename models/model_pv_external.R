...
control_series <- pageviews %>%
  filter(country_code != 'MX', referer_class == "external") %>%
...
save(model_compare, file = "data/eswiki_external_pv_model_compare.RData")

# Check model compare results
load("data/eswiki_external_pv_model_compare.RData")
...
# best model is 24, MAPE = 0.1009226
...
ggsave("eswiki_external_pv_impact.png", plot = p, path = 'docs/figures', units = "in", dpi = 300, height = 6, width = 14)
summary(model_impact)
summary(model_impact, "report")
png('docs/figures/eswiki_external_pv_inclusion_prob.png', units = "in", res = 300, height = 8, width = 10)
...
# PE_es.wikipedia      DO_es.wikipedia
#     0.99349935           1.00000000
# GT_es.wikipedia      CO_en.wikipedia
#     0.95909591           0.99199920

# average coefficient
sort(apply(model$model$coefficients[-(1:burn),], 2, PositiveMean))
# PE_es.wikipedia      DO_es.wikipedia
#     0.193215717           0.459479593
# GT_es.wikipedia      CO_en.wikipedia
#      0.135443517          0.205767841
