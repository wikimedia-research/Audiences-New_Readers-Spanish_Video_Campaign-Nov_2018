...
# eswiki pv from Mexico

cat("Data preparation...\n")
test_series <- pageviews %>%
  filter(country_code == 'MX', project == 'es.wikipedia') %>%
  group_by(date) %>%
...
save(model_compare, file = "data/include_mx/eswiki_pv_model_compare.RData")

# Check model compare results
load("data/include_mx/eswiki_pv_model_compare.RData")
...
# best model is 9 MAPE=0.07383401
...
p <- plot(model_impact) +
  ...
  labs(title = "Impact of the video campaign in 10 weeks on Spanish Wikipedia page views from Mexico",
  subtitle = "Vertical dashed line represents the start date of the campaign 15 November 2018")
ggsave("eswiki_pv_impact.png", plot = p, path = 'docs/figures/include_mx', units = "in", dpi = 300, height = 6, width = 14)
summary(model_impact)
summary(model_impact, "report")
png('docs/figures/include_mx/eswiki_pv_inclusion_prob.png', units = "in", res = 300, height = 8, width = 10)
...
sort(colMeans(model$model$coefficients[-(1:burn),] != 0))
 # EC_es.wikipedia      MX_es.wikibooks
 #      0.99449945           1.00000000

# average coefficient
sort(apply(model$model$coefficients[-(1:burn),], 2, PositiveMean))
 # EC_es.wikipedia      MX_es.wikibooks
 #      0.359008003           0.584474346
