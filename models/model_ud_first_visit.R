...
control_series <- unique_devices %>%
  filter(country_code != 'MX', type == 'first_visit') %>%
...
save(model_compare, file = "data/eswiki_first_visit_ud_model_compare.RData")

# Check model compare results
load("data/eswiki_first_visit_ud_model_compare.RData")
...
# best model is 16 MAPE=0.07948503
...
ggsave("eswiki_first_visit_ud_impact.png", plot = p, path = 'docs/figures', units = "in", dpi = 300, height = 6, width = 14)
summary(model_impact)
summary(model_impact, "report")
png('docs/figures/eswiki_first_visit_ud_inclusion_prob.png', units = "in", res = 300, height = 8, width = 10)
...
sort(colMeans(model$model$coefficients[-(1:burn),] != 0))
  # Dominican.Republic_es.wikipedia       Ecuador_es.wikipedia
  #                     1.00000000                  0.99969997

# average coefficient
sort(apply(model$model$coefficients[-(1:burn),], 2, PositiveMean))
   # Dominican.Republic_es.wikipedia       Ecuador_es.wikipedia
  #                     0.683143772                 0.257679066
