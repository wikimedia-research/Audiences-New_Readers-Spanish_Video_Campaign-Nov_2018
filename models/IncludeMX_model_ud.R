...
save(model_compare, file = "data/include_mx/eswiki_ud_model_compare.RData")

# Check model compare results
load("data/include_mx/eswiki_ud_model_compare.RData")
...
# best model is 17 MAPE=0.04998086
...
ggsave("eswiki_ud_impact.png", plot = p, path = 'docs/figures/include_mx', units = "in", dpi = 300, height = 6, width = 14)
summary(model_impact)
summary(model_impact, "report")
png('docs/figures/include_mx/eswiki_ud_inclusion_prob.png', units = "in", res = 300, height = 8, width = 10)
...
sort(colMeans(model$model$coefficients[-(1:burn),] != 0))
     #          Venezuela_es.wikipedia     Dominican.Republic_es.wiktionary
     #                      0.99249925                           0.99679968
     # Dominican.Republic_es.wikipedia                  Mexico_es.wikibooks
     #                      0.99879988                           1.00000000
     #            Mexico_es.wiktionary
     #                      1.00000000

# average coefficient
sort(apply(model$model$coefficients[-(1:burn),], 2, PositiveMean))
     #          Venezuela_es.wikipedia     Dominican.Republic_es.wiktionary
     #                      0.1043121152                           -0.2626591114
     # Dominican.Republic_es.wikipedia                  Mexico_es.wikibooks
     #                      0.2912306385                           0.3735381426
     #            Mexico_es.wiktionary
     #                      0.5987605808
